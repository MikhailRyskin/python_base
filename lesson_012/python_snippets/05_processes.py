...
#
# Очень похоже на многопоточное, но порождает процессы операционной системы.
...
# И изменения объектов в памяти одного процесса никак не отражается в памяти другого.
# Потоки же имеют доступ ко всем объектам в памяти одного процесса, именно поэтому мы могли
# в разных потоках изменять один и тот же объект (используя блокировки, конечно же).
...
# в момент порождения нового процесса ПОЛНОСТЬЮ копируется состояние памяти процесса-родителя.
...
# новый интерпретатор, с чистой памятью и процесс-потомок наследует только параметры запуска процесса.
...
# Делается это с помощью класса Pipe (труба). С его помощью можно передавать объект от процесса к процессу.
